...
BEGIN_MESSAGE_MAP(CSimpleMFCDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_SETTING, &CSimpleMFCDlg::OnBnClickedBtnSetting)
	ON_BN_CLICKED(IDC_BTN_RESET, &CSimpleMFCDlg::OnBnClickedBtnReset)
	ON_BN_CLICKED(IDC_BTN_RANDOM, &CSimpleMFCDlg::OnBnClickedBtnRandom)
	ON_BN_CLICKED(IDC_BTN_REPEAT, &CSimpleMFCDlg::OnBnClickedBtnRepeat)
	ON_MESSAGE(WM_THREAD_ACTIVE, &CSimpleMFCDlg::OnThreadActive)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
END_MESSAGE_MAP()
...
//현재 픽셀 위치 확인
bool CSimpleMFCDlg::IsInCircle(CPoint Point, CPoint Center, int nRadius)
{
	bool bRet = false;
...
//원 이미지 생성
void CSimpleMFCDlg::MakeImageCircle(unsigned char* pszImage, CPoint Center, int nRadius, int nGray)
{
...
//범위 난수 생성
int CSimpleMFCDlg::GetRandomNumber(int min, int max)
{
	return min + (rand() % (max - min + 1));
}

//3개 좌표로 그릴 원 중앙 계산
CPoint CSimpleMFCDlg::CalcCircleCenter(CPoint DotA, CPoint DotB, CPoint DotC)
{
	CPoint Center;
...
//원의 선 위에 있는지 확인
bool CSimpleMFCDlg::IsOnLine(CPoint Dot, CPoint Center, int nRadius)
{
	bool bRet = false;
...
//외곽선 있는 원 그리기
void CSimpleMFCDlg::MakeImageCircleOutline(unsigned char* pszImage, CPoint Center, int nRadius, int nGray)
{
	int x = Center.x - nRadius;
	int y = Center.y - nRadius;
	int nPitch = m_Image.GetPitch();
...
//멤버변수 점 3개 이용해서 그리기
void CSimpleMFCDlg::MakeImageCircleOutlineWithDots(unsigned char* pszImage)
{
...
//점 그리기
void CSimpleMFCDlg::MakeImageDot(unsigned char* pszImage, CPoint Point)
{
...
//저장된 점 그리기
void CSimpleMFCDlg::MakeImageDots(unsigned char* pszImage)
{
...
//점 초기화
void CSimpleMFCDlg::RemoveDots()
{
	m_Dots.clear();
}

//랜덤 점 생성
void CSimpleMFCDlg::GenerateImageRandomDots(unsigned char* pszImage)
{
	RemoveDots();
	SetClearImage();
	for(int i = 0; i < 3; i++)
...
//화면 내 위치 검증
bool CSimpleMFCDlg::ValidImagePos(CPoint point)
{
	bool bRet = false;
...
//세팅 변경시 그림 다시 그리기
void CSimpleMFCDlg::MakeImageCurSettings(unsigned char* pszImage)
{
	SetClearImage();
	MakeImageDots(pszImage);
...
//화면 지우기
void CSimpleMFCDlg::ClearScreen()
{
	Invalidate();
	UpdateWindow();
}
...
//깨끗한 이미지 생성
void CSimpleMFCDlg::SetClearImage()
{
	if (m_nBpp == 8) {
...
//화면 그리기
void CSimpleMFCDlg::DrawScreen(unsigned char* pszImage)
{
	CClientDC dc(this);
...
//클릭 지점 확인
bool CSimpleMFCDlg::IsItDot(CPoint point)
{
	bool bRet = false;
...
//점, 정원 반복 생성
DWORD CSimpleMFCDlg::RepeatGenerateCircles() {
 ...
 */
 ...
 //클릭시 점 설정
void CSimpleMFCDlg::OnBnClickedBtnSetting()
{
	unsigned char* pszImage = (unsigned char*)m_Image.GetBits();
...
//리셋
void CSimpleMFCDlg::OnBnClickedBtnReset()
{
	RemoveDots();
	SetClearImage();
	DrawScreen((unsigned char*)m_Image.GetBits());
...
//랜덤
void CSimpleMFCDlg::OnBnClickedBtnRandom()
{
	unsigned char* pszImage = (unsigned char*)m_Image.GetBits();
...
//반복
void CSimpleMFCDlg::OnBnClickedBtnRepeat()
{
	thread RepeatThread;
...
//마우스 Down 이벤트
void CSimpleMFCDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	unsigned char* pszImage = (unsigned char*)m_Image.GetBits();
	m_bLeftClick = true;
...
//마우스 Up 이벤트
void CSimpleMFCDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	m_bLeftClick = false;
	m_bDot = false;
	m_Target = (-1, -1);
...
//마우스 Move 이벤트
void CSimpleMFCDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	unsigned char* pszImage = (unsigned char*)m_Image.GetBits();
...
//Thread Acvtive 메시지
LRESULT CSimpleMFCDlg::OnThreadActive(WPARAM wParam, LPARAM lParam)
{
